...
public class TestGame implements Game {
    ...
    public static void main(String[] args) {
        new Window("Test", 640, 360, new TestGame()).run();
    }
    ...
    public void loop(Renderer renderer, Input input) {
        ...
        if (input.isMouseButtonDown(1)) {
            renderer.setPixel(input.getMouseX(), input.getMouseY());
            renderer.setPixel(input.getMouseX() + 1, input.getMouseY());
            renderer.setPixel(input.getMouseX() - 1, input.getMouseY());
            renderer.setPixel(input.getMouseX(), input.getMouseY() + 1);
            renderer.setPixel(input.getMouseX(), input.getMouseY() - 1);
        }
